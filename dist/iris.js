(function (global, factory) {
	...
	*/
	...
	USE(function(module){
	    ...
	    var Gun$$1 = (SEA.window||{}).Gun || USE((typeof common == "undefined"?'.':'')+'./gun', 1);
	    Gun$$1.SEA = SEA;
	    SEA.GUN = SEA.Gun = Gun$$1;

	    module.exports = SEA;
	  })(USE, './sea');
	USE(function(module){
	    var Gun$$1 = USE('./sea').Gun;
	...
	USE(function(module){
	    ...
	    User.prototype = (function(){ function F(){} F.prototype = Gun$$1.chain; return new F() }()); // Object.create polyfill
	    User.prototype.constructor = User;

	    // let's extend the gun chain with a `user` function.
	    // only one user can be logged in at a time, per gun instance.
	    Gun$$1.chain.user = function(pub){
	      var gun = this, root = gun.back(-1), user;
	      if(pub){ return root.get('~'+pub) }
	      if(user = root.back('user')){ return user }
	      var root = (root._), at = root, uuid = at.opt.uuid || Gun$$1.state.lex;
	...
	USE(function(module){
	    ...
	    User.prototype.create = function(alias, pass, cb, opt){
	      ...
	      cb = cb || noop;
	      if(cat.ing){
	        cb({err: Gun$$1.log("User is already being created or authenticated!"), wait: true});
	        return gun;
	      }
	      cat.ing = true;
	      opt = opt || {};
	      ...
	      act.a = function(pubs){
	        ...
	        if(pubs && !opt.already){
	          ...
	          var ack = {err: Gun$$1.log('User already created!')};
	          cat.ing = false;
	          cb(ack);
	          gun.leave();
	          return;
	        }
	        act.salt = Gun$$1.text.random(64); // pseudo-randomly create a salt, then use PBKDF2 function to extend the password with it.
	      ...
	      act.g = function(auth){ var tmp;
	        ...
	        root.get('~@'+alias).put(Gun$$1.obj.put({}, tmp, Gun$$1.val.link.ify(tmp))); // next up, we want to associate the alias with the public key. So we add it to the alias list.
	    ...
	    User.prototype.auth = function(alias, pass, cb, opt){
	      ...
	      act.a = function(data){
	        ...
	        if(!data.pub){
	          var tmp = [];
	          Gun$$1.node.is(data, function(v){ tmp.push(v); });
	          return act.b(tmp);
	        }
	      ...
	      act.c = function(auth){
	        ...
	        if(Gun$$1.text.is(auth)){ return act.c(Gun$$1.obj.ify(auth)) } // in case of legacy
	      ...
	      act.g = function(pair){
	        ...
	        at.sea = act.pair;
	        cat.ing = false;
	        try{if(pass && !Gun$$1.obj.has(Gun$$1.obj.ify(cat.root.graph['~'+pair.pub].auth), ':')){ opt.shuffle = opt.change = pass; } }catch(e){} // migrate UTF8 & Shuffle!
	        ...
	        try{
	          ...
	          Gun$$1.log("Your 'auth' callback crashed with:", e);
	        }
	      };
	      act.z = function(){
	        // password update so encrypt private key using new pwd + salt
	        act.salt = Gun$$1.text.random(64); // pseudo-random
	      ...
	      act.err = function(e){
	        var ack = {err: Gun$$1.log(e || 'User cannot be found!')};
	        cat.ing = false;
	        cb(ack);
	      };
	    ...
	    User.prototype.delete = async function(alias, pass, cb){
	      ...
	      try {
	        ...
	        Gun$$1.log('User.delete failed! Error:', e);
	      }
	      return gun;
	    };
	...
	USE(function(module){
	    ...
	    function each(msg){ // TODO: Warning: Need to switch to `gun.on('node')`! Do not use `Gun.on('node'` in your apps!
	      ...
	      Gun$$1.node.is(msg.put, function(val, key, node){
	        // only process if SEA formatted?
	        var tmp = Gun$$1.obj.ify(val) || noop;
	        if(u !== tmp[':']){
	    ...
	    function security(msg){
	      ...
	      if(msg.put){
	        ...
	        each.node = function(node, soul){
	          if(Gun$$1.obj.empty(node, '_')){ return check['node'+soul] = 0 } // ignore empty updates, don't reject them.
	          Gun$$1.obj.map(node, each.way, {soul: soul, node: node});
	        };
	        ...
	        each.alias = function(val, key, node, soul){ // Example: {_:#~@, ~@alice: {#~@alice}}
	          ...
	          if('~@'+key === Gun$$1.val.link.is(val)){ return check['alias'+key] = 0 } // in fact, it must be EXACTLY equal to itself
	        ...
	        each.pubs = function(val, key, node, soul){ // Example: {_:#~@alice, ~asdf: {#~asdf}}
	          ...
	          if(key === Gun$$1.val.link.is(val)){ return check['pubs'+soul+key] = 0 } // and the ID must be EXACTLY equal to its property
	        ...
	        each.pub = function(val, key, node, soul, pub, user){ var tmp; // Example: {_:#~asdf, hello:'world'~fdsa}}
	          ...
	          SEA.verify(SEA.opt.pack(val,key,node,soul), pub, function(data){ var rel;
	            ...
	            if((rel = Gun$$1.val.link.is(data)) && pub === SEA.opt.pub(rel)){
	              (at.sea.own[rel] = at.sea.own[rel] || {})[pub] = true;
	            }
	            check['user'+soul+key] = 0;
	            each.end({ok: 1});
	    ...
	    SEA.opt.prep = function(d,k, n,s){ // prep for signing
	      return {'#':s,'.':k,':':SEA.opt.parse(d),'>':Gun$$1.state.is(n, k)};
	    };
	    SEA.opt.pack = function(d,k, n,s){ // pack for verifying
	      if(SEA.opt.check(d)){ return d }
	      var meta = (Gun$$1.obj.ify(d)||noop), sig = meta['~'];
	      return sig? {m: {'#':s,'.':k,':':meta[':'],'>':Gun$$1.state.is(n, k)}, s: sig} : d;
	    };
	    SEA.opt.unpack = function(d, k, n){ var tmp;
	      ...
	      var soul = Gun$$1.node.soul(n), s = Gun$$1.state.is(n, k);
