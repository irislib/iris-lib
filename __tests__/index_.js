...
describe('local index', async () => {
  ...
  test('like & unlike', async () => {
    ...
    let msgReactions = await i.gun.back(-1).get('messagesByHash').get(msg.getHash()).get('reactions').once().then();
    ...
    msgReactions = await i.gun.back(-1).get('messagesByHash').get(msg.getHash()).get('reactions').once().then();
